...
#include "RtosEval.h" // in-process backend
#include "Space.h"
...
void AutotuneWorker::runRtos() {
  ...
  autotune::RtosEval rtos(m_p.rtosBin, m_p.suffix);
  if (!rtos.binExists()) {
  ...
  rtos.setExcitation(m_p.rollout.stepUs, m_p.rollout.tetherK, m_p.rollout.hold,
                     m_p.rollout.ret, m_p.rollout.settle,
                     m_p.rollout.excite == autotune::Excitation::Chirp ? 1 : 0,
                     m_p.rollout.chirpF0, m_p.rollout.chirpF1);
  // Tune the loaded airframe: hand the vsim_ctl_geometry_t blob to the backend,
  // which applies it to both the in-process physics and the firmware mix.
  if (m_p.hasGeometry) {
    const QByteArray blob(reinterpret_cast<const char *>(&m_p.geometry),
                          int(sizeof(m_p.geometry)));
    QString gerr;
    if (rtos.setGeometry(blob, &gerr))
      emit log(
          QStringLiteral("applied vehicle geometry (physics + firmware mix)."));
    else
      emit log(
          QStringLiteral("warning: %1 — tuning the reference quad.").arg(gerr));
  } else {
    emit log(QStringLiteral(
        "note: no vehicle geometry — tuning the reference quad."));
  }
  ...
  AutotuneEngine engine(
      ...
      [&](const QVector<double> &xq) -> std::optional<double> {
        ...
        const QString rateStr = m_p.rtosRateCost
                                    ? QStringLiteral(" rate[roll=%1 pitch=%2]")
                                          .arg(last.rollRateCost, 0, 'f', 2)
                                          .arg(last.pitchRateCost, 0, 'f', 2)
                                    : QString();
        emit log(
            QStringLiteral("  eval [%1]  cost=%2  angle: roll=%3 pitch=%4%5%6"
                           "  (%7x)%8")
                .arg(gs.join(QStringLiteral(", ")))
                ...
                .arg(m_p.tuneYaw ? QStringLiteral(" yaw=%1").arg(last.yawCost,
                                                                 0, 'f', 2)
                                 : QString())
                .arg(rateStr)
  ...
  connect(&engine, &AutotuneEngine::evaluated, this,
          &AutotuneWorker::evaluated);
...
void AutotuneWorker::runSysId() {
  ...
  emit log(QStringLiteral(
      ...
      "gains analytically (loop-shaping). No optimizer search, no tracking "
      "cost "
  ...
  const double hold =
      std::max(1.5, m_p.rollout.hold); // long enough to fit a lag
  rtos.setExcitation(m_p.rollout.stepUs, m_p.rollout.tetherK, hold,
                     m_p.rollout.ret, m_p.rollout.settle, /*waveform=*/1, f0,
                     f1);
  ...
  if (m_p.hasGeometry) {
    ...
    if (rtos.setGeometry(blob, &gerr))
      ...
      emit log(QStringLiteral("warning: %1 — identifying the reference quad.")
                   .arg(gerr));
  } else {
    emit log(QStringLiteral(
        "note: no vehicle geometry — identifying the reference quad."));
  }
  ...
  QString err;
  if (!rtos.captureRate(autotune::RtosEval::Gains{}, m_p.rollout.seed, raw,
                        &err)) {
  ...
  if (!prp.ok) {
    emit failed(QStringLiteral("plant identification failed (roll R^2=%1, "
                               "pitch R^2=%2) — the chirp did "
                               "not excite the rate loop. Try a wider chirp "
                               "band or a larger amplitude.")
  ...
  emit log(
      QStringLiteral("roll/pitch plant: K=%1, tau=%2 ms, actuator BW=%3 Hz "
  ...
  emit log(
      QStringLiteral("design: crossover wc=%1 rad/s (%2 Hz, %3 of actuator BW) "
  ...
  if (m_p.tuneYaw) {
    ...
    if (py.ok) {
      ...
      emit log(QStringLiteral(
                   "yaw plant: K=%1, tau=%2 ms (R^2=%3) -> yaw_rate_kp=%4")
                   .arg(py.K, 0, 'g', 4)
      ...
      emit log(
          QStringLiteral("warning: yaw plant fit failed — leaving yaw_rate_kp "
  ...
  for (int i = 0; i < int(names.size()); ++i) {
    ...
    if (n == "rate_kp")
      ...
      bestX[i] = rpg.angle_kp;
    else if (n == "yaw_rate_kp")
      bestX[i] = (yawRateKp >= 0 ? yawRateKp : 0.0);
  }
  ...
  vg.rate_kp = rpg.rate_kp;
  vg.rate_ki = rpg.rate_ki;
  vg.rate_kd = rpg.rate_kd;
  vg.angle_kp = rpg.angle_kp;
  if (yawRateKp >= 0)
    vg.yaw_rate_kp = yawRateKp;
  ...
  if (!m_cancel) {
    ...
    if (vr.ok && !vr.starved) {
      ...
      emit log(QStringLiteral(
                   ...
                   .arg(m_p.tuneYaw ? QStringLiteral(" yaw=%1").arg(vr.yawCost,
                                                                    0, 'f', 2)
                                    : QString())
      ...
      emit log(
          QStringLiteral("verification doublet did not score (%1)")
