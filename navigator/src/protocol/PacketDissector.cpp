...
QString sysStateName(uint16_t s) {
  ...
  case 0x100:
    return "CALIBRATING";
  default:
    return QString("0x%1").arg(s, 0, 16);
  }
}
...
QString PacketDissector::typeName(uint8_t type) {
  ...
  default:
    return QString("UNKNOWN(0x%1)").arg(type, 1, 16);
  }
}
...
QString PacketDissector::originName(uint8_t o) {
  ...
  case 0x08:
    return "EST_PERF";
  default:
    return QString("0x%1").arg(o, 2, 16, QChar('0'));
  }
}
      ...
      case 0x01: { // CALIBRATION
        ...
        for (int i = 0; p + 3 + 4 * i + 4 <= n && i < (length - 3) / 4; i++)
          pl.addChild(QString("Value %1").arg(i),
                      QString::number(f32(p + 3 + 4 * i), 'f', 4),
                      p + 3 + 4 * i, 4);
        break;
      }
    ...
    case 0x9: { // PERF_STATS fragment
      if (length >= 8) {
        ...
        if (section == 0 && b + 72 <= n) { // GLOBAL
          ...
          for (int i = 0; i < 17; i++)
            g.addChild(gn[i], QString::number(u32(b + 4 + 4 * i)),
                       b + 4 + 4 * i, 4);
        } else if (section == 1) { // TASKS rows (20 B)
          for (int i = 0; i < count && b + i * 20 + 20 <= n; i++) {
            const int r = b + i * 20;
            DissectField &t = pl.addChild(QString("Task #%1").arg(raw[r]),
            ...
            t.addChild("Max burst cyc", QString::number(u32(r + 16)), r + 16,
                       4);
          }
        } else if (section == 2) { // FIFOS rows (8 B)
          for (int i = 0; i < count && b + i * 8 + 8 <= n; i++) {
            const int r = b + i * 8;
            DissectField &f = pl.addChild(perfFifoName(raw[r]),
                                          QString("%1/%2, drops %3")
                                              .arg(u16(r + 2))
                                              .arg(u16(r + 4))
                                              .arg(u16(r + 6)),
...
QString PacketDissector::summary(const QByteArray &data) {
  ...
  case 0x1:
    return "IMU full";
  case 0x2:
    return "IMU Δ (compressed)";
  case 0x4:
    return ok && p + 12 <= n ? QString("roll %1  pitch %2  yaw %3")
                                   .arg(f32(p), 0, 'f', 1)
                                   .arg(f32(p + 4), 0, 'f', 1)
                                   .arg(f32(p + 8), 0, 'f', 1)
                             : "Attitude";
  case 0x5:
    return "RC channels";
  case 0x7:
    return "LOG: " + asAscii(data.mid(p, qMin<int>(length, 48)));
  case 0x8:
    return "Motor telemetry";
  case 0xA:
    return ok ? QString("TaskName id=%1").arg(raw[p]) : "TaskName";
  case 0x3:
  ...
  case 0x9: { // PERF_STATS
    ...
    const char *s = raw[p + 1] == 0   ? "GLOBAL"
                    ...
                    : raw[p + 1] == 2 ? "FIFOS"
                                      : "?";
    return QString("Perf %1 idx=%2 n=%3")
        .arg(s)
        .arg(raw[p + 2])
        .arg(raw[p + 3]);
  }
