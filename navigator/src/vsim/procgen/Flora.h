...
struct FloraParams {
  ...
  float bladesPerCell =
      2.0f;                // carpet density (camera-facing fraction fills gaps)
  float heightMean = 1.4f; // blade height: normal distribution [m]
  float heightStdDev = 0.3f;
  float flowerFrac =
      0.01f; // sparse flowers (more reads as litter in a dense field)
};
...
struct GrassLook {
  ...
  float rootDarkness =
      0.14f; // ambient-occlusion floor at the blade base (lower = darker)
  float faceCameraFrac =
      0.35f; // fraction of blades turned broadside to the camera
};
