...
inline QVector<PidSetCmd>
autotuneGainsToCommands(const QStringList &names,
  ...
  const auto val = [&](const QString &k) { return float(v.value(k, 0.0)); };

  QVector<PidSetCmd> cmds;
  for (int axis = 0; axis <= 1; ++axis) { // roll, pitch
    if (has("rate_kp"))
      cmds.push_back(
          {1, axis, val("rate_kp"), val("rate_ki"), val("rate_kd"), 0.0f});
    if (has("angle_kp"))
