...
LogPanel::LogPanel(QWidget *parent) : QGroupBox("Log", parent) {
  ...
  m_autoScroll->setToolTip(tr("Pin the view to the newest line"));
  m_pauseBtn = new ui::GhostButton(tr("Pause"), this);
  m_pauseBtn->setCheckable(true);
  m_pauseBtn->setFixedWidth(70);
  m_pauseBtn->setToolTip(
      tr("Freeze the scrollback view (disk log keeps writing)"));
  ...
  clearBtn->setToolTip(
      tr("Empty the scrollback (persistent log on disk is kept)"));
...
QString LogPanel::stampFor(const Entry &e) const {
  ...
  case Local:
  default:
    return QDateTime::fromMSecsSinceEpoch(e.wallMs).toString(
