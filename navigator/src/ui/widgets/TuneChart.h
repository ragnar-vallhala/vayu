...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    if (baseline_ > 0 && finite(baseline_)) {
      vmax = std::max(vmax, baseline_);
      vmin = std::min(vmin, baseline_);
    }
    if (vmin > vmax)
      vmin = 0;
    ...
    auto X = [&](int i) {
      return plot.left() + plot.width() * (n <= 1 ? 0.5 : double(i) / (n - 1));
    };
    auto Y = [&](double v) {
      const double cv = std::min(v, vmax);
      return plot.bottom() -
             plot.height() * (cv - vmin) / std::max(1e-6, vmax - vmin);
    };
    ...
    f.setPixelSize(9);
    p.setFont(f);
    p.drawText(QRectF(0, plot.top() - 4, 30, 12), Qt::AlignRight,
               QString::number(vmax, 'f', 0));
    p.drawText(QRectF(0, plot.bottom() - 8, 30, 12), Qt::AlignRight,
               QString::number(vmin, 'f', 0));

    if (baseline_ > 0 && finite(baseline_)) {
      p.setPen(QPen(base, 1.0, Qt::DashLine));
      p.drawLine(QPointF(plot.left(), Y(baseline_)),
                 QPointF(plot.right(), Y(baseline_)));
    }
    // cost scatter
    p.setPen(Qt::NoPen);
    for (int i = 0; i < n; ++i) {
    ...
    QPolygonF poly;
    for (int i = 0; i < n; ++i)
      poly << QPointF(X(i), Y(best_[i]));
    p.drawPolyline(poly);

    p.setPen(txt);
    p.drawText(QRectF(plot.left(), H - 14, plot.width(), 12), Qt::AlignHCenter,
               QString("eval %1   best %2")
                   .arg(n)
                   .arg(best_.isEmpty() ? 0.0 : best_.last(), 0, 'f', 2));
  }
