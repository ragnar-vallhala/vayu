...
Op opOf(const QString &s) {
  ...
  if (s == ">=")
    return Op::Ge;
  if (s.compare("contains", Qt::CaseInsensitive) == 0)
    return Op::Contains;
  return Op::Invalid;
}
...
QVector<Tok> lex(const QString &src, bool *ok) {
  ...
  *ok = true;
  ...
  while (i < n) {
    ...
    if (c == '&' && at(i + 1) == '&') {
      out.push_back({Tok::AndOp, "&&"});
      i += 2;
      continue;
    }
    if (c == '|' && at(i + 1) == '|') {
      out.push_back({Tok::OrOp, "||"});
      i += 2;
      continue;
    }
    if (c == '=' && at(i + 1) == '=') {
      out.push_back({Tok::Cmp, "=="});
      i += 2;
      continue;
    }
    if (c == '!' && at(i + 1) == '=') {
    ...
    if (c.isDigit()) {
      int j = i;
      while (j < n && (src[j].isLetterOrNumber()))
        ++j; // 0x.. too
      out.push_back({Tok::Num, src.mid(i, j - i)});
      i = j;
      continue;
    }
    if (c.isLetter() || c == '_') {
      int j = i;
      while (j < n &&
             (src[j].isLetterOrNumber() || src[j] == '_' || src[j] == '.'))
        ++j;
      out.push_back({Tok::Word, src.mid(i, j - i)});
      i = j;
      continue;
    }
    *ok = false;
    return out; // unexpected char
  }
...
struct Parser {
  ...
  Pred comparison() {
    ...
    if (cur().kind == Tok::Cmp)
      op = opOf(cur().text);
    else if (cur().kind == Tok::Word && opOf(cur().text) == Op::Contains)
      op = Op::Contains;
    else {
      ok = false;
      return {};
    ...
    eat();
    if (cur().kind != Tok::Word && cur().kind != Tok::Num &&
        cur().kind != Tok::Str) {
      ok = false;
      return {};
    }
    ...
    return [f, op, low, num, numOk](const FilterCtx &c) -> bool {
      ...
      case Field::Dev:
        return numOk && numCmp(c.dev, op, num);
      case Field::Size:
        return numOk && numCmp(c.len, op, num);
      case Field::Origin:
        if (!numOk)
          return false;
        if (c.origin < 0)
