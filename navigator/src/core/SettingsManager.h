...
struct GcsSettings {
  ...
  friend QDataStream &operator<<(QDataStream &out, const GcsSettings &s) {
    out << s.syncPeriodMs << s.graphWindowSec << s.graphDropoutRate
        << s.autoReconnect << s.recordOnConnect << s.recentViewsCount << s.theme
        << s.defaultTransport << s.defaultPort << s.defaultBaud
        ...
        << s.traceWidth << s.antialias << s.logDirectory << s.timestampMode
        << s.maxLogLines << s.exportOnDisconnect << s.toastNotifications
        << s.audioAlerts << s.confirmBeforeArm << s.simPropAudio
        << s.packetBufferRows << s.crcCheck;
    return out;
  }
  ...
  friend QDataStream &operator>>(QDataStream &in, GcsSettings &s) {
    ...
    if (!in.atEnd())
      in >> s.autoReconnect;
    if (!in.atEnd())
      in >> s.recordOnConnect;
    if (!in.atEnd())
      in >> s.recentViewsCount;
    if (!in.atEnd())
      in >> s.theme;
    if (!in.atEnd())
      in >> s.defaultTransport;
    if (!in.atEnd())
      in >> s.defaultPort;
    if (!in.atEnd())
      in >> s.defaultBaud;
    if (!in.atEnd())
      in >> s.reconnectIntervalSec;
    if (!in.atEnd())
      in >> s.linkLossTimeoutMs;
    if (!in.atEnd())
      in >> s.unitSystem;
    if (!in.atEnd())
      in >> s.angleUnit;
    ...
    if (!in.atEnd())
      in >> s.maxLogLines;
    if (!in.atEnd())
      in >> s.exportOnDisconnect;
    if (!in.atEnd())
      in >> s.toastNotifications;
    if (!in.atEnd())
      in >> s.audioAlerts;
    if (!in.atEnd())
      in >> s.confirmBeforeArm;
    if (!in.atEnd())
      in >> s.simPropAudio;
    if (!in.atEnd())
      in >> s.packetBufferRows;
    if (!in.atEnd())
      in >> s.crcCheck;
    return in;
  }
