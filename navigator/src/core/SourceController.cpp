...
void SourceController::teardown(SourceState s) {
  switch (s) {
  case SourceState::Fc:
    if (m_hooks.teardownFc)
      m_hooks.teardownFc();
    break;
  case SourceState::Sim:
    if (m_hooks.teardownSim)
      m_hooks.teardownSim();
    break;
  case SourceState::Autotune:
    if (m_hooks.teardownAutotune)
      m_hooks.teardownAutotune();
    break;
  case SourceState::Replay:
    if (m_hooks.teardownReplay)
      m_hooks.teardownReplay();
    break;
...
bool SourceController::transition(SourceState to,
  ...
  if (m_hooks.applyFeed)
    m_hooks.applyFeed(m_state);
