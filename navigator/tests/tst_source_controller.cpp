...
struct Spy {
  ...
  SourceController::Hooks hooks() {
    ...
    h.setupFc = [this](const QString &, int) { log << "su:Fc"; return setupOk; };
    h.setupSim = [this] { log << "su:Sim"; return setupOk; };
    h.setupAutotune = [this] { log << "su:Autotune"; return setupOk; };
    h.setupReplay = [this](const QString &) { log << "su:Replay"; return setupOk; };
  ...
  static QString name(SourceState s) {
    switch (s) {
    case SourceState::Idle: return "Idle";
    case SourceState::Fc: return "Fc";
    case SourceState::Sim: return "Sim";
    case SourceState::Autotune: return "Autotune";
    case SourceState::Replay: return "Replay";
    }
    return "?";
  }
};

const SourceState kAll[] = {SourceState::Idle, SourceState::Fc, SourceState::Sim,
                            SourceState::Autotune, SourceState::Replay};

void drive(SourceController &c, SourceState s) {
  switch (s) {
  case SourceState::Idle: c.requestIdle(); break;
  case SourceState::Fc: c.requestFc("COM1", 115200); break;
  case SourceState::Sim: c.requestSim(); break;
  case SourceState::Autotune: c.requestAutotune(); break;
  case SourceState::Replay: c.requestReplay("flight.bin"); break;
  }
}
  ...
  for (SourceState from : kAll) {
    for (SourceState to : kAll) {
      ...
      if (from != SourceState::Idle)
        QVERIFY2(spy.log.contains(td), qPrintable(td + " from " + Spy::name(from)));
