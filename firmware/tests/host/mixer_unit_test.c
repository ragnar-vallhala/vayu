...
static void reference_mix(float thr, float roll, float pitch, float yaw,
                          float out[4]) {
  for (int i = 0; i < 4; i++)
...
int main(void) {
  mixer_t mx;
  if (!mixer_set_geometry(&mx, PX, PY, SP, 4)) {
    printf("geometry FAILED\n");
    return 1;
  }
  ...
  printf("Test 3: saturation — airmode preserves roll, uniform scaler discards "
         "it\n");
  {
    ...
    printf("    airmode RP  : realized roll = %.3f  thrust = %.3f  motors "
           "{%.2f %.2f %.2f %.2f}\n",
           r_rp[MIX_ROLL], r_rp[MIX_THRUST], m_rp[0], m_rp[1], m_rp[2],
           m_rp[3]);
    printf("    airmode off : realized roll = %.3f  thrust = %.3f  motors "
           "{%.2f %.2f %.2f %.2f}\n",
           r_off[MIX_ROLL], r_off[MIX_THRUST], m_off[0], m_off[1], m_off[2],
           m_off[3]);

    check("airmode RP delivers the full demanded roll",
          near(r_rp[MIX_ROLL], w[MIX_ROLL]));
    check("uniform scaler LOSES roll authority (< demanded)",
          r_off[MIX_ROLL] < w[MIX_ROLL] - 1e-3f);
    check("airmode raised collective to make room",
          r_rp[MIX_THRUST] > w[MIX_THRUST] + 1e-3f);
    check("all motors in [0,1]", m_rp[0] >= -1e-6f && m_rp[3] <= 1.0f + 1e-6f);
  }
  ...
  printf("Test 5: roll authority is flat across the throttle sweep\n");
  {
    ...
    for (int mode = 0; mode < 2; mode++) {
      ...
      printf("   worst = %.2f\n", (double)worst);
      if (mode)
        check("RP keeps >=95% of the demanded roll at EVERY throttle",
              worst >= 0.95f);
      else
        check("(baseline) uniform scaler collapses somewhere in the sweep",
              worst < 0.5f);
    }
  }
