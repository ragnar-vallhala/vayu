...
bool mixer_set_geometry(mixer_t *mx, const float *pos_x, const float *pos_y,
  ...
  if (mx == NULL || n == 0 || n > MIXER_MAX_MOTORS)
    return false;
  mx->n = n;
  mx->idle_floor = 0.0f;
  ...
  float thr_sum = 0.0f;
  for (uint8_t i = 0; i < n; i++)
    thr_sum += mx->Bpinv[i][MIX_THRUST];
  if (m_fabsf(thr_sum) < 1e-9f)
    return false;
...
void mixer_allocate(const mixer_t *mx, const float w[MIX_NW], float *motor,
  ...
  if (mx->airmode == MIXER_AIRMODE_DISABLED) {
    ...
    if (mxv > 1.0f) {
      float k = (1.0f - thr) / (mxv - thr);
      if (k < scale)
        scale = k;
    }
    if (scale < 1.0f)
      for (uint8_t i = 0; i < n; i++)
        motor[i] = thr + scale * (motor[i] - thr);
  } else {
