...
/* HSL_STATUS: high-speed SD recorder status (ring position, wraps, drops). */
void navlink_tx_hsl_status(void);                        /* v2 NOTCH_STATUS */
...
void navlink_tx_sysid_sample(uint16_t start, uint16_t total, uint16_t hz,
                             ...
                             const int16_t *gyro);
void navlink_tx_imu_compressed(
    const uint16_t delta_f16[10]);                   /* v2 IMU_COMPRESSED */
