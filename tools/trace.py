...
Walks the vayu-owned source trees (src/, include/, tools/) and the
...
against the MOD-SUB-NNN rows in docs/firmware/requirements.md, and
emits docs/firmware/trace.md.

Per docs/firmware/requirements.md §5.3 — vendor exemption:
...
CLI:
    tools/trace.py                  Regenerate docs/firmware/trace.md.
    tools/trace.py --check          CI gate. Unknown ID fails (Phase 1).
                                    Missing implementer/verifier warns.
    tools/trace.py --check --strict Phase-5 mode: missing implementer or
...
REQUIREMENTS_MD = REPO_ROOT / "docs" / "firmware" / "requirements.md"
TRACE_MD = REPO_ROOT / "docs" / "firmware" / "trace.md"
...
def emit_trace_md(reqs, impls, verifs, out_path: Path) -> None:
    ...
    lines.append("")
    lines.append("> Generated by `tools/trace.py`. Do not edit by hand.")
    lines.append("> Source of truth: `docs/firmware/requirements.md`.")
    lines.append("")
