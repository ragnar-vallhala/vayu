...
public:
    void setConfig(const WindConfig& c) {
        cfg_ = c;
        if (cfg_.turb_tau <= 0.0f) cfg_.turb_tau = 1.0f;  // guard div-by-zero
    }
    ...
    void reset() { t_ = 0.0f; turb_ = Vec3(0.0f, 0.0f, 0.0f); last_ = Vec3(0.0f, 0.0f, 0.0f); }

    // Advance by dt and return the instantaneous world wind [m/s] NED.
    const Vec3& step(float dt) {
        if (!cfg_.enable) { last_ = Vec3(0.0f, 0.0f, 0.0f); return last_; }
        t_ += dt;
        ...
        if (cfg_.gust_amp != 0.0f && cfg_.gust_period > 0.0f) {
            ...
            const Vec3 dir = (hn > 1e-6f)
                                 ? Vec3(cfg_.steady.x() / hn, cfg_.steady.y() / hn, 0.0f)
                                 : Vec3(1.0f, 0.0f, 0.0f);
            const float amp = cfg_.gust_amp *
                              std::sin(2.0f * kPi * t_ / cfg_.gust_period);
            gust = dir * amp;
        }
        ...
        if (cfg_.turb_sigma > 0.0f) {
            ...
            turb_ = Vec3(a * turb_.x() + k * norm_(rng_),
                         a * turb_.y() + k * norm_(rng_),
