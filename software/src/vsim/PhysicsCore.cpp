...
PhysicsCore::PhysicsCore() = default;
...
PhysicsCore::Deriv PhysicsCore::derive(const RigidBodyState& s,
  ...
  // Linear: rotate body force into world, add gravity, subtract drag.
  ...
  // Attitude: q_dot = 0.5 * q * (0, omega_b)
  // QQuaternion API: scalar first.
  ...
  // Angular: Euler's equations with diagonal inertia + angular drag.
...
void PhysicsCore::step(const Vec3& force_b, const Vec3& torque_b, float dt) {
  // Classic RK4. Wrench is held constant across the step (zero-order
  // hold from the motor model), so we just re-evaluate the derivative
  // at the four sample points.
  ...
  Deriv k;
  k.d_pos   = (k1.d_pos   + 2.0f*k2.d_pos   + 2.0f*k3.d_pos   + k4.d_pos)   * (1.0f/6.0f);
  k.d_vel   = (k1.d_vel   + 2.0f*k2.d_vel   + 2.0f*k3.d_vel   + k4.d_vel)   * (1.0f/6.0f);
  k.d_omega = (k1.d_omega + 2.0f*k2.d_omega + 2.0f*k3.d_omega + k4.d_omega) * (1.0f/6.0f);
  k.d_quat  = Quat(
  ...
  state_.att.normalize();   // drift-correct after Euler step on q

  groundClamp();
}

// Hard floor at z == ground_z (remember: NED, so larger z is lower).
// When the body's CoM would dip below the ground, snap z to ground_z
// and zero out any downward (positive-z) velocity. The attitude is
// left alone -- the user wanted "simple landing clamp, no tipping" --
// so the drone can sit at any roll/pitch indefinitely without the
// ground tilting it. The bridge isn't simulating a contact patch;
// landing gear shape is a render-only detail.
void PhysicsCore::groundClamp() {
  if (state_.pos_w.z() > params_.ground_z) {
    state_.pos_w.setZ(params_.ground_z);
    if (state_.vel_w.z() > 0.0f) {
      // Optional bounce; default restitution = 0 just stops the body.
      state_.vel_w.setZ(-state_.vel_w.z() * params_.ground_restitution);
    }
    // Light tangential friction to bleed off sliding when grounded.
