...
def gen_c_frame_decls(d):
    ...
    p(" * COMMAND_ACK it emits on the handler's behalf (spec §12.1). */")
    p("typedef struct { uint8_t result; uint8_t progress; int32_t result_param2; } navlink_ack_t;")
    p("")
...
def gen_c_frame_defs(d):
    ...
    p(" * h->send. The dispatch calls this for every such message, so a consumer")
    p(" * cannot silently drop the ack — it only supplies the result (or, with no")
    p(" * handler, the dispatch reports UNSUPPORTED). */")
    p("static void navlink_emit_ack(const navlink_handlers_t *h, uint32_t command,")
    ...
    for m in d["messages"]:
        ...
        if requires_ack(m):
            ...
            p("        else {")
            p("            _ack.result = NAVLINK_COMMAND_RESULT_UNSUPPORTED; _ack.progress = 0; _ack.result_param2 = 0;")
            p(f"            if (h->on_default) h->on_default(h->ctx, hdr, NAVLINK_MSGID_{m['name']}, pay, len);")
            p("        }")
            p(f"        navlink_emit_ack(h, NAVLINK_MSGID_{m['name']}, a.req_seq, _ack);")
            p("        break;")
            p("    }")
        else:
            p(f"    case NAVLINK_MSGID_{m['name']}:")
