...
int main(void) {
    ...
    for (const char *p = s; *p; p++) navlink_crc_accumulate((uint8_t)*p, &crc);
    assert(crc == 0x6F91);

    /* Local round-trips. */
    {
        navlink_attitude_euler_t a = { .roll = 0.5f, .pitch = -0.25f, .yaw = 1.0f,
                                       .rollspeed = 0.1f, .pitchspeed = 0.2f, .yawspeed = 0.3f };
        navlink_attitude_euler_wire_t w;  navlink_attitude_euler_from_aligned(&w, &a);
        uint8_t buf[64]; size_t n = navlink_attitude_euler_pack(buf, &w);
        assert(n == sizeof w && n == NAVLINK_WIRE_SIZE_ATTITUDE_EULER);
        navlink_attitude_euler_wire_t w2; navlink_attitude_euler_unpack(&w2, buf, n);
        navlink_attitude_euler_t a2;      navlink_attitude_euler_to_aligned(&a2, &w2);
        assert(a2.roll == a.roll && a2.yawspeed == a.yawspeed);
    }
    {
        navlink_command_ack_t k = { .command = 0x123456, .req_seq = 7, .result = 0,
                                    .progress = 100, .result_param2 = -3 };
        navlink_command_ack_wire_t w; navlink_command_ack_from_aligned(&w, &k);
        navlink_command_ack_t k2;     navlink_command_ack_to_aligned(&k2, &w);
    ...
    assert(navlink_msg_info(1034) && strcmp(navlink_msg_info(1034)->name, "PERF_GLOBAL") == 0);
    assert(navlink_msg_info(0x424242) == NULL);

    /* Short unpack zero-fills the rest (spec §5.6). */
    {
        uint8_t two[2] = { 7, 0 };
        navlink_param_value_wire_t w; navlink_param_value_unpack(&w, two, 2);
        navlink_param_value_t a; navlink_param_value_to_aligned(&a, &w);
        assert(a.param_id[0] == 7 && a.count == 0 && a.generation == 0 && a.value[7] == 0);
    }
    /* Oversize unpack is clamped to the wire size (no overrun). */
    {
        ...
        uint8_t out[64]; size_t n = navlink_attitude_euler_pack(out, &w);
        assert(n == NAVLINK_WIRE_SIZE_ATTITUDE_EULER);
    }
    /* Signed i8 array round-trip (motor spin = +1/-1). */
    {
        navlink_cmd_set_motor_geometry_t g; memset(&g, 0, sizeof g);
        g.spin[0] = 1; g.spin[1] = -1; g.spin[2] = 1; g.spin[3] = -1;
        navlink_cmd_set_motor_geometry_wire_t w; navlink_cmd_set_motor_geometry_from_aligned(&w, &g);
