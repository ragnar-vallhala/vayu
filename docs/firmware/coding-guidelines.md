...
| R8.6  | See [`docs/note_to_myself.md`](../note_to_myself.md): **no locks in high-frequency loop paths**. The rate loop, IMU ISR, and motor task must not contend on any mutex. Use lock-free queues or single-writer/single-reader buffers. |
...
| R12.3 | A static-analysis baseline shall be maintained; new violations are not allowed, existing ones are tracked and burned down. | 🟡 cppcheck baseline is clean and CI hard-gates it (--error-exitcode=1) — fixed a real null-deref (calibration_task) + a wrong-IRQ-detach in del_handler, suppressed one false positive. clang-tidy genuine findings fixed + ruleset tuned, still report-only (host-build parse caveat). |
