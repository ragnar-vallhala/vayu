...
| R12.3 | A static-analysis baseline shall be maintained; new violations are not allowed, existing ones are tracked and burned down. | 🟡 clang-tidy burn-down done — genuine findings fixed (branch-clone, switch-default, MS_TO_TICKS redefine), ruleset tuned to a clean analysable baseline; cppcheck baseline + flip-to-failing pending. |
