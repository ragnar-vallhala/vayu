...
| SYS-SAFE-002  | RC loss behaviour                                                                                                                           | On loss of valid RC frames for > 1.0 s, the vehicle shall transition to the FAILSAFE state and command zero motor output. No autonomous RTL/LAND. **🟡 gap** — current firmware reacts only on iBUS `channel[0] == 0` deadband (see COMM-RC-002); no elapsed-time watchdog. | (top level)  | Test (SITL) + Demonstration        |
| SYS-SAFE-003  | Sensor-fault failsafe                                                                                                                       | If the attitude estimator flags its state as degraded for > 100 ms, the vehicle shall transition to FAILSAFE. **🟡 gap** — estimator does not currently emit a degraded flag (EST-MAH-002 unimplemented). | (top level)  | Test (SITL with fault injection)   |
...
| SYS-SAFE-005  | Arming preconditions                                                                                                                        | The vehicle shall refuse to transition from STANDBY to ARMED unless: estimator converged, RC valid, calibration current, throttle stick at minimum. **🟡 gap** — current firmware only checks RC `ch[4] > 1500` + `ch[2] < 1100`; no estimator-convergence gate, no calibration-freshness gate. | (top level)  | Test (unit + SITL)                 |
...
| EST-MAH-002  | Fault-sample rejection             | The estimator shall reject any IMU sample whose validity flag is false (per SNS-IMU-002) and shall raise an `estimator_degraded` flag after 100 ms of continuous rejection.                                                                                                                            | SYS-SAFE-003     | Test (unit, fault injection) | **🟡 gap** — no degraded flag emitted today. |
...
| COMM-RC-002   | RC loss detection                  | The RC subsystem shall raise `rc_loss` if no valid frame is received for > 100 ms. **🟡 gap** — current firmware only inspects `channel[0] == 0`; no elapsed-time watchdog (see SYS-SAFE-002).                                                            | SYS-SAFE-002     | Test (unit + SITL)          |
...
| COMM-CH-002   | UART backpressure                  | A write that would exceed the active buffer's 512 B capacity shall return `ERROR` and increment a `tx_overflow` counter. The counter shall be surfaced through telemetry. **🟡 gap** — return ERROR is implemented; counter not yet exposed.             | (process)        | Test (unit, fault injection) |
...
| COMM-CMD-002  | Command payload validation          | Every command handler shall validate `argc ≥ expected_argc` and `payload_length ≥ argc × 4 + 3` before reading args.                                                                                                                                       | (process)        | Test (unit, fuzz)           | **🟡 gap** — current calibration handler reads payload without length check. |
| COMM-CMD-003  | CMD_SET_PID                         | The command dispatcher shall accept `CMD_SET_PID` (0x000A) carrying live PID gain updates. **🟡 gap** — stub today; payload schema and persistence policy still to be defined.                                                                            | SYS-CTRL-001     | Test (unit + SITL)          |
...
| COMM-TEL-002  | Heartbeat cadence                   | The vehicle shall emit a heartbeat packet at ≥ 1 Hz. **🟡 gap** — current firmware emits every 150 ticks × 6 ms = 900 ms ⇒ 1.11 Hz (mislabelled "1 Hz" in `telemetry_task.c:50`).                                                                          | SYS-TEL-001      | Test (SITL)                 |
