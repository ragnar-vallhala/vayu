...
| SNS-CAL-101   | Accel ellipsoid calibration | Accel calibration shall accept ~12 prompted-but-pose-tolerant static holds (6 faces + 6 edges/corners; stillness-gated, no axis-dominance requirement), fit a magnitude-only ellipsoid (`calib_fit_ellipsoid`) to recover offset + a full 3×3 matrix, rescale so corrected ‖a‖ = g, and commit only on a successful fit. | SYS-CAL-002      | Test (bench + host unit)  |
