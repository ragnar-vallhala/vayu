...
void SimController::stepOnce(const std::array<float, 4> &duty, float dt) {
  ...
  if (vibe_gain_ > 0.0f) {
    float mean_cmd =
        0.25f *
        (std::clamp(duty[0], 0.0f, 1.0f) + std::clamp(duty[1], 0.0f, 1.0f) +
         std::clamp(duty[2], 0.0f, 1.0f) + std::clamp(duty[3], 0.0f, 1.0f));
...
ImuSample SimController::sampleImu(float dt) {
  return sensors_.sample(phys_.state(), last_a_world_, phys_.params().gravity,
                         dt);
}
