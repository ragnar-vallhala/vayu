...
PhysicsCore::Deriv PhysicsCore::derive(const RigidBodyState &s,
  ...
  d.d_quat = Quat(0.5f * q_dot.scalar(), 0.5f * q_dot.x(), 0.5f * q_dot.y(),
                  0.5f * q_dot.z());
...
RigidBodyState PhysicsCore::advance(const RigidBodyState &s, const Deriv &k,
  ...
  n.att = Quat(s.att.scalar() + k.d_quat.scalar() * dt,
               s.att.x() + k.d_quat.x() * dt, s.att.y() + k.d_quat.y() * dt,
...
void PhysicsCore::step(const Vec3 &force_b, const Vec3 &torque_b, float dt) {
  ...
  Deriv k;
  k.d_pos =
      (k1.d_pos + k2.d_pos * 2.0f + k3.d_pos * 2.0f + k4.d_pos) * (1.0f / 6.0f);
  k.d_vel =
      (k1.d_vel + k2.d_vel * 2.0f + k3.d_vel * 2.0f + k4.d_vel) * (1.0f / 6.0f);
  k.d_omega =
      (k1.d_omega + k2.d_omega * 2.0f + k3.d_omega * 2.0f + k4.d_omega) *
      (1.0f / 6.0f);
  k.d_quat = Quat(
      (k1.d_quat.scalar() + 2 * k2.d_quat.scalar() + 2 * k3.d_quat.scalar() +
       k4.d_quat.scalar()) *
          (1.0f / 6.0f),
      (k1.d_quat.x() + 2 * k2.d_quat.x() + 2 * k3.d_quat.x() + k4.d_quat.x()) *
          (1.0f / 6.0f),
      (k1.d_quat.y() + 2 * k2.d_quat.y() + 2 * k3.d_quat.y() + k4.d_quat.y()) *
          (1.0f / 6.0f),
      (k1.d_quat.z() + 2 * k2.d_quat.z() + 2 * k3.d_quat.z() + k4.d_quat.z()) *
          (1.0f / 6.0f));
...
void PhysicsCore::applyContact(const Vec3 &cpb, const Vec3 &nW, float pen,
                               float restitution, Vec3 &posCorr,
                               float &maxPen) {
...
static const Vec3 kFootprint[4] = {
    Vec3(0.13f, 0.13f, 0), Vec3(-0.13f, 0.13f, 0), Vec3(-0.13f, -0.13f, 0),
    Vec3(0.13f, -0.13f, 0)};
    ...
    for (const Vec3 &cpb : kFootprint) {
      ...
      if (o.type == 1) { // sphere
        ...
        if (rxy < rad && std::fabs(lp.z()) < hh) {
          ...
          if (pr < pz && rxy > 1e-5f) {
            nL = Vec3(lp.x() / rxy, lp.y() / rxy, 0);
            pen = pr;
          } else {
            nL = Vec3(0, 0, lp.z() >= 0 ? 1.f : -1.f);
            pen = pz;
          }
          hit = true;
        }
        ...
        if (std::fabs(lp.x()) < hx && std::fabs(lp.y()) < hy &&
          ...
          if (px <= py && px <= pz) {
            nL = Vec3(lp.x() >= 0 ? 1.f : -1.f, 0, 0);
            pen = px;
          } else if (py <= pz) {
            nL = Vec3(0, lp.y() >= 0 ? 1.f : -1.f, 0);
            pen = py;
          } else {
            nL = Vec3(0, 0, lp.z() >= 0 ? 1.f : -1.f);
            pen = pz;
...
void PhysicsCore::sanitize() {
  ...
  const bool ok =
      fin3(state_.pos_w) && fin3(state_.vel_w) && fin3(state_.omega_b) &&
      std::isfinite(state_.att.scalar()) && std::isfinite(state_.att.x()) &&
      std::isfinite(state_.att.y()) && std::isfinite(state_.att.z());
  if (!ok) {
    const Vec3 keep = fin3(state_.pos_w)
                          ? state_.pos_w
...
void PhysicsCore::groundClamp(float dt) {
  ...
  if (grounded_) {
    ...
    const float tilt2 =
        tiltW.x() * tiltW.x() + tiltW.y() * tiltW.y() + tiltW.z() * tiltW.z();
