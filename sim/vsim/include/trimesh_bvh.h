...
namespace detail {
struct BuildTri {
  float cmin[3], cmax[3], centroid[3];
  uint32_t idx;
};
...
inline int32_t build(std::vector<Node> &nodes, std::vector<BuildTri> &bt,
  ...
  if (cmax[1] - cmin[1] > ext) {
    axis = 1;
    ext = cmax[1] - cmin[1];
  }
  if (cmax[2] - cmin[2] > ext) {
    axis = 2;
  }
...
inline std::vector<uint8_t> buildSerialized(const float *verts, uint32_t nverts,
                                            const uint32_t *tris,
                                            uint32_t ntris, bool doubleSided) {
  ...
  h.magic = kMagic;
  h.version = kVersion;
  h.vertex_count = nverts;
  h.triangle_count = ntris;
  h.node_count = static_cast<uint32_t>(nodes.size());
  h.flags = doubleSided ? kFlagDoubleSided : 0u;
  for (int k = 0; k < 3; ++k) {
    h.aabb_min[k] = nodes[0].bmin[k];
    h.aabb_max[k] = nodes[0].bmax[k];
  }
  h.vertices_off = vOff;
  h.triangles_off = tOff;
  h.tri_perm_off = pOff;
  h.nodes_off = nOff;
  h.total_size = total;
  std::memcpy(blob.data(), &h, sizeof(h));
  if (nverts)
    std::memcpy(blob.data() + vOff, verts,
                uint64_t(nverts) * 3 * sizeof(float));
  if (ntris)
    std::memcpy(blob.data() + tOff, tris,
                uint64_t(ntris) * 3 * sizeof(uint32_t));
  if (ntris)
    std::memcpy(blob.data() + pOff, perm.data(),
                uint64_t(ntris) * sizeof(uint32_t));
...
struct Bvh {
  ...
  static Bvh fromBytes(const uint8_t *base, size_t len) {
    ...
    if (h->total_size > len)
      return r;
    if (h->nodes_off + uint64_t(h->node_count) * sizeof(Node) > len)
      return r;
    r.h = h;
