...
int main() {
  ...
  // --- closestPointOnTriangle: face / edge / vertex regions ---
  {
    ...
    CHECK(near(closestPointOnTriangle(Vec3(0.5f, 0.5f, 5), a, b, c),
               Vec3(0.5f, 0.5f, 0)),
          "closest: above-face projects onto face");
  ...
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < N; ++j) {
      tris.push_back(vid(i, j));
      tris.push_back(vid(i + 1, j));
      tris.push_back(vid(i + 1, j + 1));
      tris.push_back(vid(i, j));
      tris.push_back(vid(i + 1, j + 1));
      tris.push_back(vid(i, j + 1));
    }
  const uint32_t nverts = uint32_t(verts.size() / 3),
                 ntris = uint32_t(tris.size() / 3);
  ...
  CHECK(std::fabs(bvh.h->aabb_min[0] + 5.0f) < 1e-4f &&
            std::fabs(bvh.h->aabb_max[0] - 5.0f) < 1e-4f,
        "root AABB spans the floor");
