...
static std::vector<uint8_t> floorBlob(float half, int n) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j) {
            t.push_back(vid(i, j)); t.push_back(vid(i + 1, j)); t.push_back(vid(i + 1, j + 1));
            t.push_back(vid(i, j)); t.push_back(vid(i + 1, j + 1)); t.push_back(vid(i, j + 1));
        }
...
int main() {
    ...
    // --- 1) Drone dropped onto the floor rests on it, doesn't fall through ---
    {
        ...
        p.reset(s);
        for (int i = 0; i < 4000; ++i) p.step(Vec3(0, 0, 0), Vec3(0, 0, 0), 0.001f);
    ...
    // --- 2) Dropped off-centre on a tilted contact -> acquires tilt (tips) ---
    {
        ...
        for (int i = 0; i < 200; ++i) p.step(Vec3(0, 0, 0), Vec3(0, 0, 0), 0.001f);
        const float vz = p.state().pos_w.z();
        printf("  after impact z = %.3f, vel.z = %.2f\n", vz, p.state().vel_w.z());
        CHECK(p.state().pos_w.z() < 0.2f, "stopped at/above the floor after impact");
    ...
    // --- 3) No world mesh -> falls to the ground plane (z=5) ---
    {
        ...
        p.reset(s);
        for (int i = 0; i < 4000; ++i) p.step(Vec3(0, 0, 0), Vec3(0, 0, 0), 0.001f);
        CHECK(p.state().pos_w.z() > 4.5f, "with no mesh, falls through to ground_z");
    }
