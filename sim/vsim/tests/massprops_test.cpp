...
int main(int argc, char **argv) {
  ...
  // --- Cube: side a, mass m -> I = (1/6) m a^2 on diagonal, products 0.
  {
    ...
    check(close(mp.ixy, 0, 1e-5) && close(mp.ixz, 0, 1e-5) &&
              close(mp.iyz, 0, 1e-5),
          "products~0", mp.ixy, 0);
  }
  ...
  //     matches the centered case (parallel-axis handled internally).
  {
    ...
    check(close((mp.com - c).length(), 0, 1e-4), "com at offset",
          (mp.com - c).length(), 0);
  ...
  //     is unchanged (translation-invariant about the CoM).
  {
    ...
    for (auto &v : soup)
      v -= a.com; // editor's loadAndCompute shift
    auto b = computeMassProperties(soup, m);
    check(close(b.com.length(), 0, 1e-4), "recentered com~0", b.com.length(),
          0);
  ...
  // --- Mat3 inverse: I * I^-1 == identity for a non-diagonal tensor.
  {
    ...
    for (int r = 0; r < 3; ++r)
      for (int col = 0; col < 3; ++col) {
        ...
        if (std::fabs(P.at(r, col) - want) > 1e-4f)
          ok = false;
      }
  ...
  if (argc > 1) {
    ...
    QString err;
    vsim::LoadedMesh mesh =
        vsim::loadMesh(QString::fromUtf8(argv[1]), 1.0f, &err);
