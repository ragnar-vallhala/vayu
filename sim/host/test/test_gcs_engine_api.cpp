...
int main() {
  ...
  for (int i = 0; i < 1200; ++i)
    rtos_engine_run_step(); // ~1.2 s wall (paced)
  ...
  std::printf("    (telemetry: %zu bytes in %d callbacks; tick %u -> %u)\n",
              g_telem_bytes.load(), g_telem_calls.load(), p0.hdr.seq_no,
              p1.hdr.seq_no);
  ...
  vsim_pose_frame_t pb{};
  vsim_inproc_get_pose(&pb);
  ...
  for (int i = 0; i < 300; ++i)
    rtos_engine_run_step();
  vsim_pose_frame_t pa{};
  vsim_inproc_get_pose(&pa);
  ...
  geo.mass = 1.0f;
  geo.inertia[0] = geo.inertia[4] = 0.012f;
  geo.inertia[8] = 0.022f;
  for (int i = 0; i < 4; ++i) {
    geo.motors[i].axis[2] = -1.0f; // body -Z (up in NED)
    geo.motors[i].spin = (i % 2) ? 1.0f : -1.0f;
    geo.motors[i].k_thrust = 1e-5f;
    geo.motors[i].k_moment = 1e-7f;
    geo.motors[i].max_omega = 2500.0f;
    geo.motors[i].tau = 0.0125f;
  ...
  for (int i = 0; i < 100; ++i)
    rtos_engine_run_step();
  vsim_pose_frame_t pg{};
  vsim_inproc_get_pose(&pg);
  check("sim still advancing after geometry push",
        pg.hdr.seq_no > pa.hdr.seq_no);
