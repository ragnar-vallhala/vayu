...
static const char *pwm_fifo_path(void) {
  static char p[128];
  return path_suffixed(VSIM_FIFO_PWM, p, sizeof p);
}
static const char *uart2_advert_path(void) {
  static char p[128];
  return path_suffixed(VAYU_UART2_PTY_PATH, p, sizeof p);
}
static const char *uart2_log_path(void) {
  static char p[128];
  return path_suffixed(VAYU_UART2_LOG_PATH, p, sizeof p);
}
...
static void pwm_fifo_ensure_open(void) {
  if (pwm_fifo_fd >= 0 || pwm_fifo_open_failed)
    return;
    ...
    if (fd >= 0 && grantpt(fd) == 0 && unlockpt(fd) == 0) {
      ...
      if (p) {
        ...
        fprintf(stderr, "host_navhal: UART2 pty open, slave=%s\n",
                uart2_slave_path);
        fprintf(stderr,
                "host_navhal: connect the GCS to %s "
                "(or read %s)\n",
...
} /* RTOS build: host_rtos_port.c drives the idle yield */
#endif

hal_status_t hal_uart_write_char(hal_uart_t uart, char c) {
  if (uart == HAL_UART_2)
    uart2_write_bytes(&c, 1);
  return HAL_OK;
}
...
hal_status_t hal_uart_enable_interrupt(hal_uart_t uart, uint8_t rx,
                                       uint8_t tx) {
  (void)uart;
  (void)rx;
  (void)tx;
